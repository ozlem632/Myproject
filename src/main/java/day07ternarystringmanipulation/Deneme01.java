...
public class Deneme01 {
    public static void main(String[] args) {
        ...
        //BIR PASWORDUN GECERLI OLMASI ICIN EN AZ BIR SPACE ICERMELI
        //bUYUK HARFLE BASLAMALI

        // EN AZ BIR SEMBOL ICERMELI
        //EN AZ IKI TANE NOKTALAMA ISARETI OLMALI
        // SON HARFI * O

        //EGER OYLEYSE  harika degilse berbat yzyazdirin

        String contr = "Ozlem ,.*";

        boolean sart1 = contr.replaceAll("[0-9A-Za-z \\p{Punct}]", "").length() > 0;
        boolean sart2 = contr.charAt(0)=='O';
        boolean sart3 = contr.replaceAll("[A-Za-z0-9]" , "").length()>0;
        boolean sart4 = contr.replaceAll("[A-Za-z0-9]", "").length() < 2;
        boolean sart5 = contr.charAt(contr.length() - 1) == '*';
        boolean gecerli = sart1 && sart2 && sart3 && sart4 && sart5 ;
        if (gecerli) {
